...
app.post('/webhooks/dora', (req, res) => {
  ...
  switch (event.event) {
    case 'delivery.completed':
      ...
      // Access receiver details
      event.data.receiver_detail.forEach(receiver => {
        console.log(`Delivered to: ${receiver.customer_name}`);
        console.log(`Address: ${receiver.customer_address}`);
      });
      break;
    
    case 'delivery.picked_up':
      // Send SMS notification
      const receiver = event.data.receiver_detail[0];
      if (receiver) {
        const phone = receiver.customer_phone_code + receiver.customer_phone_number;
        sendSMS(phone, 'Your package is on the way!');
      }
      break;
  }
...
def dora_webhook():
    ...
    if event['event'] == 'delivery.completed':
        ...
        # Access receiver details
        for receiver in event['data']['receiver_detail']:
            customer_name = receiver['customer_name']
            customer_address = receiver['customer_address']
            print(f"Delivered to {customer_name} at {customer_address}")
            
    elif event['event'] == 'delivery.picked_up':
        receiver = event['data']['receiver_detail'][0] if event['data']['receiver_detail'] else None
        if receiver:
            phone = receiver['customer_phone_code'] + receiver['customer_phone_number']
            notify_customer(phone)
