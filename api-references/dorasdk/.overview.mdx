...
description: "This guide will walk you through creating and running your first task on Dora."
---
...
## Setting up the Dora SDK

You can use the Dora CLI to create a new Dora project (or add Dora to an existing Node.js project)

```bash
npx Dora@latest init --language typescript
```

The CLI will create an `Dora/` directory for you. This is where all of you Dora tasks will live.

## Creating a Task

Each task is defined as its own file within the `Dora/` directory.
...
1. Onboard a demo user by making a request to Dora's demo API
...
import Dora from "@Dorahq/node";
import fetch from 'node-fetch';

const task = new Dora.Task({
  ...
  run: async (input, context) => {
    ...
    // For the purposes of this demo, we'll just make a call to Dora's demo API
    const response = await fetch("https://demo.joinDora.com/api/onboard", {
      method: "POST",
      headers: {
...
To deploy your tasks, you can use the `Dora deploy` command.

```bash
npx Dora@latest deploy
```


You can also self host your Dora instance. 
Documentation for self-hosting your Dora tasks is coming soon. 
In the meantime, you can reach out to us at founders@joinDora.com for instructions on how to self-host your Dora tasks.

## Running a Task

Once you’ve created and deployed your task, you can access its UI on the Dora dashboard.
...
<Frame>
  <img src="images/create-store.png" />
</Frame>
...
To edit a task, simply edit the task's configuration file and re-deploy your tasks to Dora.

## (Optional) Integrations

Dora comes with a built-in [Slack integration](/nodesdk/integrations/slack).
To enable it, navigate to the **Integrations** tab on the **Settings** screen, find the Slack integration and click **Connect**.
You'll be redirected to a page asking you grant Dora permission to access your Slack workspace.
