...
After signing up, you will generate store token by creating a store name, this store name is a blueprint of of your existing website(s) or physical store. You can ignore this step if your store is part of our list of supported [store integrations](../stores/create-store-token).  
...
Explore our ever-expanding list of supported [store integrations](../stores/create-store-token). We continually expand this list to ensure that you can effortlessly synchronize and process incoming customer orders using the automation capabilities available on Dora.
